...
export const addPeminjaman = async (req, res) => {
    ...
    const formattedBorrowDate = new Date(borrow_date).toISOString(); //ini untuk merubah format tanggal
        ...
        try {
            ...
            res.status(201).json({
                ...
                data: {
                    ...
                    qty: result.qty,
                    borrow_date: result.borrow_date.toISOString().split("T")[0], //  (YYYY-MM-DD)
        ...
        try {
            ...
            res.status(201).json({
                ...
                data: {
                    ...
                    id_barang: result.id_barang,
                    qty: result.qty,
                    return_date: result.return_date.toISOString().split("T")[0], //  (YYYY-MM-DD)
                    status: result.status,
                },
            });
        } catch (error) {
...
export const usageReport = async (req, res) => {
    ...
    try {
        ...
        const items = await prisma.barang.findMany({ //mengfilter barang 
            where: {
                AND: [
        ...
        const analysis = items.map((item) => { //mengkelompokkan barang 
            ...
            return {
                group: group_by === 'category' ? item.category : item.location, // Grup berdasarkan kategori barang
        ...
        res.status(500).json({
            status: "error",
            message: "Terjadi kesalahan saat memproses laporan penggunaan.",
            error: error.message,
        });
    }
};
